...
function buildRouterTs(postRoutes, pageRoutes) {
	let result = `import {createRouter, createWebHistory, RouteRecordRaw} from "vue-router";const routes: RouteRecordRaw[]=[{name:"page",path:"/",component:()=>import("@/Page.vue"),children:[` + pageRoutes.map(x => `{name: "${x.filename.toLowerCase()}", path: "/${x.filename}", component:()=>import("@/pages/${x.filename}.vue"),meta:{filename:"${x.filename}",title:"${x.title}",uuid:"${x.uuid}",isStandalone:true}}`).join(',') + `]},{name:"posts",path:"/posts",component:()=>import("@/Post.vue"),children:[` + postRoutes.map(r => `{name:"post-${r.filename.toLowerCase()}",path:"${r.filename}",component:()=>import("@/posts/${r.filename}.vue"),meta:{filename:"${r.filename}",title:"${r.title}",uuid:"${r.uuid}",isStandalone:false}}`).join(',');
...
async function buildPageComponent(fm, fileparsed, wordcount) {
	...
	const result = await page.evaluate(
		async (fm, wordcount) => {
			...
			if (h && !fm.standalone) {
				const metabar = `<div class="metabar">` + `<div class="metabar-item">${fm.date}</div>` + (fm.cate ? `<div class="metabar-item">${fm.cate}</div>` : '') + `<div class="metabar-item">约 ${getReadingMinutes(wordcount, fm.cate)} 分钟读完</div>` + `</div>`;
